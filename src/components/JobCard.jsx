...
export function JobCard({ job }) {
    ...
    const { titulo: title, descripcion: descripcion, empresa: company, ubicacion: location, sueldo: salary ,  publicado: publicado} = job
    const { technology: technology, modalidad: modality, nivel: nivel } = job.data
        ...
        <>
            ...
            <article onClick={() => navigateTo(`/job/${job.id}`)} className='job-card flex flex-col sm:flex-row items-start gap-4 p-4 rounded-xl border border-border-light bg-white hover:border-primary transition-colors cursor-pointer'>
                ...
                <header className="flex-1">
                    ...
                    <p className="text-sm text-text-light-secondary">{descripcion}</p>
                    <p className="text-sm text-primary font-semibold">{company}</p>
                    <p className="text-sm text-text-light-secondary mt-1">{location} | {modality} | {nivel}</p>
